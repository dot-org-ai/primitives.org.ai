  ...
  describe('getConditionalActions()', () => {
    const workflow = Workflow({
      ...
      actions: [
        ...
        { order: 2, type: 'notify', description: 'Conditional', condition: 'user.premium === true' },
        { order: 3, type: 'create', description: 'Also conditional', condition: 'order.total > 100' },
      ],
    })
  ...
  describe('validateWorkflow()', () => {
    ...
    it('should fail if action has no type', () => {
      const workflow: WorkflowDefinition = {
        ...
        trigger: { type: 'manual' },
        actions: [
          { order: 1, type: '' as WorkflowAction['type'], description: 'No type' },
        ],
      }
    ...
    it('should fail if wait action has no duration', () => {
      const workflow: WorkflowDefinition = {
        ...
        trigger: { type: 'manual' },
        actions: [
          { order: 1, type: 'wait', description: 'Wait without duration' },
        ],
      }
