...
// @ts-expect-error  H1: a bare Passed is not a Grant (no residual/validUntil/predicate)
const _h1 = commitFromGrant(passA, 200, stackA, () => {})

// ----- (H2) tripwire-less delegation: a Grant MUST carry its predicate (the checker) -----
// @ts-expect-error  H2: grantFrom requires the tripwire predicate (4th arg) — delegation without a checker is impossible
const _h2 = grantFrom(decisionA, 5_000, 9_999)

// ----- (H3, H×F) the standing envelope is NON-PORTABLE across tenants -----
// @ts-expect-error  H3: A-principal grant cannot commit against B's Stack (principal mismatch)
const _h3 = commitFromGrant(grantA, 200, stackB, () => {})

// ----- (H4) the Grant brand is UNFORGEABLE — you cannot hand-build a blank check -----
// @ts-expect-error  H4: object literal is missing the [GRANT] brand — cannot forge a Grant
...
// @ts-expect-error  H5: the tripwire must be a predicate function, not a constant boolean
