// =====================================================================================
// FC2 COVARIANT-PARAMETER CANARY (ADR 0081 · Epic-A2 · issue #933).
//
...
// covariant defect is caught (a spent `@ts-expect-error` over a non-erroring line is TS2578).
//
...
//     the guarded line COMPILES — so the strip un-spends the `@ts-expect-error` and THIS harness
...
// @ts-expect-error  authorityGate.D: a 'quality' seat cannot check a 'merge' gate
...
// @ts-expect-error  adversarialGate.D: a 'quality' seat cannot sit on a 'merge' adversarial panel
...
// @ts-expect-error  resolvePending.OutcomeD: a 'dispatch' verdict cannot close a 'delivery' pending
...
// @ts-expect-error  resolvePending.Corr: a 'wo-9' verdict cannot close a 'wo-1' pending
...
// @ts-expect-error  serialize.D: a 'money' record cannot be serialized onto an 'attestation' token
...
// @ts-expect-error  remint.D: a 'money' gate cannot re-mint an 'attestation' parked token
...
// @ts-expect-error  remint.Corr: a 'soc2-2027' gate cannot re-mint a 'soc2' parked token
...
// =====================================================================================
// @ts-expect-error  authorityGate.Prin: tenant-B seat cannot check a tenant-A gate (brand)
const _agPrin = authorityGate('merge', 'c', A, perf, controllerB)
// @ts-expect-error  adversarialGate.Prin: tenant-B seat cannot sit on a tenant-A panel (brand)
