...
// @ts-expect-error  E1: verdict carrying 'wo-99999' cannot resolve a Pending correlated to 'wo-88231'
...
// @ts-expect-error  E2: a Pending is not a Commitment; assigning it claims an outcome that hasn't happened
const _e2: Commitment<void> = pendingE

// ----- (E3) the outcome verdict must match the awaited OUTCOME domain, not the command domain -----
// @ts-expect-error  E3: a Passed<'dispatch'> (command domain) cannot close a Pending awaiting 'delivery'
...
// @ts-expect-error  E4: async commit yields Pending|Escalation; it is not directly a Commitment
...
// @ts-expect-error  E5: goodDeliveryVerdict is a Passed, not a Pending — nothing to resolve
...
// @ts-expect-error  E6: Pending is not an Escalation (its .kind is 'pending', not 'escalated')
