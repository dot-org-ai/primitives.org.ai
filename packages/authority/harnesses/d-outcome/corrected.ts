...
// @ts-expect-error  D1: an Accepted (external judge) cannot enter the command/competence
...
// @ts-expect-error  D2: the customer CANNOT be modeled as a competence Seat — 'customer' is
//                   not a SeatName.
const _d2: Seat<'customer', 'quality', Principal<'A'>> = { name: 'customer', domain: 'quality', principal: A }

// @ts-expect-error  D3: distinctness (fwd) — a competence Passed is NOT an Accepted (missing [ACCEPT]+basis).
const _d3: Accepted<'brand-9', Principal<'A'>> = passDelivery

// @ts-expect-error  D4: distinctness (rev) — an Accepted is NOT a Passed (missing [PASS] brand).
const _d4: Passed<'delivery', 'brand-9', Principal<'A'>> = acceptedBrand

// @ts-expect-error  D5: PricingBasis is closed — 'subscription' is not a member.
const _d5 = outcomeGate('brand-9', A).accept(customerJudge, true, 'subscription')

// @ts-expect-error  D6: THE laundering path itself — you cannot mint a Passed by feeding the
...
// @ts-expect-error  D7: composes with E — an Accepted carrying the WRONG correlation cannot
...
// @ts-expect-error  E-closed: wrong-corr Accepted rejected even with Corr inferred (NoInfer)
...
// @ts-expect-error  R-susan: susan is not admitted -> no Agent type (round-1 A″ still fires)
type _SusanAgent = Agent<'susan', Principal<'A'>>

// @ts-expect-error  R-sod: SoD competence-distinct — a 'quality' seat cannot check a 'money' gate
