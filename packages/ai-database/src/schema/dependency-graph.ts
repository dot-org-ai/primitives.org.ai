...
export function buildDependencyGraph(schema: ParsedSchema): SchemaDepGraph {
  const nodes: Record<string, SchemaDepNode> = {}
  const edges: SchemaDepEdge[] = []
 ...
 */
export function getParallelGroups(graph: SchemaDepGraph, rootType: string): string[][] {
 ...
 */
export function getAllDependencies(graph: SchemaDepGraph, typeName: string): Set<string> {
 ...
 */
export function hasCycles(graph: SchemaDepGraph): boolean {
 ...
 */
export function visualizeGraph(graph: SchemaDepGraph): string {
