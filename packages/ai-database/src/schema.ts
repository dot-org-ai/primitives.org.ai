...
} from './schema/entity-operations.js'
        ...
        get: () => {
          ...
          if (isBackward && !field.isArray) {
            ...
            if (field.unionTypes && field.operator === '<~') {
              return (async () => {
                const singleStoredId = data[fieldName] as string | undefined
                const matchedType = data[`${fieldName}$matchedType`] as string | undefined
                if (singleStoredId) {
                  const provider = await resolveProvider()
