...
describe('AIFailureClassifier', () => {
  ...
  describe('Failure Severity Assessment', () => {
    it('should assess severity based on failure type', () => {
      expect(
        classifier.assessSeverity({ code: 'timeout', message: '', timestamp: new Date() })
      ).toBe('low')
      expect(
        classifier.assessSeverity({
      ...
      expect(
        classifier.assessSeverity({ code: 'safety_violation', message: '', timestamp: new Date() })
      ).toBe('critical')
    })
